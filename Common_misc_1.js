...
function multilangPage() {
    ...
    foot = ['<span id="inpaddrow2" style="border:solid 1px lightslategray" class="btnRoundsmall imdim"><i class="fa fa-plus-circle"/>add</span>' +
               '|{"colspan":"4","style":"text-align:right;padding:5px;"}'];
...
function googletranInit() {
    ...
    $("#gtran").remove();
    jscriptInsert("gtran", "https://translate.google.com/translate_a/element.js?cb=googleSectionalElementInit&ug=section&hl=" + $("#selCountry").val());
...
function faLoad(receiver, iconname, type) {
    ...
    if (typeof type == "undefined" && typeof iconname!="undefined" && iconname.indexOf("ui-icon") > -1) type = "jqueryui";
    else type = "awesome";
    ...
    if (type != "jqueryui")
        ...
        jsonReadAjax("imcicon", "", "", "", faLoad.iconlist, [receiver, iconname, type]);
         ...
         iconlist([], receiver, iconname, type);
    }
    function iconlist(iconlist, receiver, iconname, type) { 
        ...
        if (type == "jqueryui") {
            fa = ui;
            $(ui).each(function (a, b) {
                iconlist.push({ icon: b });
            });
        }
        ...
        dv = $("<div class='form-inline'/>").appendTo(box);;
        dv.append($("<em>selection: </em>"));


        if (type == 'jqueryui') {
            dv.append("<span></span><label id='lbIcon1' />");
        }
        else {
            dv.append("<i style='vertical-align:middle;margin-left:5px'/>&nbsp;<label id='lbIcon1' />");
            dv.append($(makeCtr(["select", "1x,;lg,fa-lg;2x,fa-2x;3x,fa-3x;4x,fa-4x;5x,fa-5x", "selIcon", "col-2|margin:0 5px 0 5px", ""])));
           
        }
       
        dv.append("<button id='btnicon' >Select</input>");
        box.append($("<hr/>"));

        //search
        var search = $("<div class='form-inline' style='margin:10px 0 10px 0'/>");
        //search.css("float", "right");
        
        var inp = $("<input class='form-control form-control-sm col-6' type='search'/>")
            , sbtn = $("<button id='inpsearch' type='button' style='margin-left:5px' class='btn btn-primary btn-sm'>search</button>")
            , typelist = $(makeCtr(["select", "select,,selected,disabled;awesome;jqueryui", "selicontype", "col-2|margin:0 5px 0 5px", ""]));
        search.append($("<em>search: </em>")).append(inp).append(typelist).append(sbtn);
        box.append(search);
      

        var ul = makelist(fa,type);
        box.append(ul);
        ...
        if (receiver != "") {
            ...
            console.log(icon,receiver,seltrim)
            $("#selIcon").val(icon.size)
            if (type == "jqueryui") {
                seltrim1 = "ui-icon " + seltrim;
                $("#lbIcon1").prev().attr("class", seltrim1);
                $("#lbIcon1").text(seltrim);
            }
            else {
        ...
        $("#selIcon").on("change", function () {
            if (type == "jqueryui") {
               // $("#lbIcon1").prev().css("margin", 20);
                //$("#lbIcon1").prev().removeClass("ui-icon");
            }
            else
                $("#lbIcon1").prev().attr("class", "fa " + $("#lbIcon1").text() + " " + $("#selIcon").val());
        })
        //icontype change
        $("#selicontype").on("change", function () {
            if ($("#selicontype").val() == "jqueryui")
                faLoad.iconlist([], receiver, iconname, $("#selicontype").val());
            else
                faLoad(receiver, iconname, $("#selicontype").val());
        });
        ...
        $("#btnicon").click(function () {
            if (receiver != "") {
                ...
                if (typeof receiver == "string")
                    rctr = $("#" + receiver);
                if(type== "jqueryui"){
                    switch (rctr.prop("tagName")) {
                        case "LABEL": case "SPAN":
                            rctr.text(combtxt);
                            rctr.prev().attr("class", "ui-icon "+combtxt);
                            break;
                        case "INPUT":
                            rctr.val(combtxt);
                            break;
                            ...
                            combtxt += " " + size;
                        console.log(selname, receiver, type)
                        switch (rctr.prop("tagName")) {
                            case "LABEL": case "SPAN":
                                rctr.text(combtxt);
                                rctr.prev().attr("class", "fa " + combtxt);
                                break;
                            case "INPUT":
                                rctr.val("fa "+ combtxt);
                                break;
        ...
        sbtn.on("click", function () {
            ...
            $('.l25.li').remove();
            var keywd = $("#inpsearch").prev().prev().val();
            fa = [];
            $(iconlist).each(function (i, k) {
                if(keywd=="")
                    fa.push(k.icon);
        ...
        setTimeout(function () {
            $("#dialog").parent().css({ "z-Index": 1331, top: 50, left: 50, "min-height": "800px" });
            iconclick(type);
            $("#selicontype").val(type);
    ...
    function iconclick(type) {
        ...
        $('.l25 li').click(function () {
            if (type == "jqueryui") {
                ...
                var selecticon = $(this).find('span').attr("class");
                console.log('here',selecticon)
                var selecticon1 = selecticon.replace("ui-icon ", "");
                $("#lbIcon1").text(selecticon1);
                $("#lbIcon1").prev().removeClass().addClass(selecticon);
            }
            else {
...
function nationalFlag(option) {
    ...
    $(ct).each(function (i, k) {
        ...
        flaglist.push(k.countryname + "," + k.langcode + "," + folder + size + "/" + k.countrycode + ".png,");
    });
        ...
        switch (inputEl.tagName) {
            case "DIV":
                if ($(inputEl).hasClass("form-inline")) {
                    swit($(inputEl).children().first(), arr);
                }
                break;
            case "SPAN":
...
function actionclick(val, datacode, dndcontain, ctrtype) {
    ...
    var seq = '1', code = "", object = "",title="act"+anum, position = "bottomright", linkfield = "", display = "show",callback="", icon = "", name = '',bstyle='', data = '', keycode = '', local = ''
        , url = '', sdata = '', script = "", saveplace = "", actiontype = "", connectstring = "", dtype = "", action = "", dataset = datacode, mapping = "", jsonscheme = [], reloadlist = [];
    ...
    if (val.hasOwnProperty("object")) object = val.object; if (val.hasOwnProperty("type")) object = val.type;
    ...
    if (val.hasOwnProperty("command")) action = val.command; if (val.hasOwnProperty("action")) action = val.action;
    ...
    if ($("#tbBtn").length == 0) {
        ...
        switch (ctrtype) {
            ...
            case "html":
                poslist = "select,,selected,disabled;out,topright;out,topleft;out,bottomleft;out,bottomright";
                var editor = ace.edit("dveditor").getValue();
                $(editor).add($(editor).find("*")).each(function (i, k) {
                    var nodename=$(k).prop("nodeName"),name;
                    if ($.inArray(nodename, ["INPUT", "BUTTON"]) > -1 | $(k).hasClass("clickable")) {
                        switch (nodename) { 
                            case "INPUT":
                                poslist += ";domobject," + $(k).val() +","+$(k).outerHTML();
                                break;
                            case "BUTTON": case "SPAN": case "DIV":
       ...
       // data1.push([makeCtr(["label", "ConnectString", , , ]), makeCtr(["span", connectstring, "spconnectstring", , ])]);
        ...
        data1.push([makeCtr(["label", "Linkfield", , "imdim", ]), makeCtr(["select:multiselect", fieldlist.join(";"), "sellinkfield", , ""])]);
        ...
        $("#selobject").on("change", function () {
            showhide($(this).val());
            console.log('chg')
            chgactionlist(ctrtype, $(this).val());
        });
        function chgactionlist(ctrtype, object) {
            switch (ctrtype) {
                case "map":
                    var evtlist = eventlistfinder(ctrtype, object)
                    selectupdate($("#selevent"), evtlist);
                    break;
            }
            ...
            function eventlistfinder(ctrtype, object) {
                ...
                , 'mousedown', 'mouseeventer', 'mouseleave', 'mousemove', 'mouseout', 'mouseover', 'mouseup', 'resize'], mapevent = [];
                switch (ctrtype) {
                    case "map":
        ...
        $("#lbactiondatacode").bind("triggerevent", function () {
            ...
            jsonReadAjax("imcdata", "", "code", $(this).text(), actionclick.actionlist, [action]);
        });
        ...
        $(aform).each(function (i, k) {
            ...
            switch (name) {
                case "seq":
                    ...
                    set["subseq"] = k[1];
                    break;
                case "title":case "code": case "buttonname": case "buttonstyle":case "data": case "keycode": case "type": case "action": case "actiontype":
    ...
    setTimeout(function () {
        ...
        if (action != "") {//$("#lbactiondatacode").text() != "" && 
            if (typeof dtt == "object")
                actionlist(dtt, action);
            else
            jsonReadAjax("imcdata", "", "code", $("#lbactiondatacode").text(), actionclick.actionlist, [action]);
        }
        else
            actionlist();
    }, 100);
    setTimeout(function () {
        if( $("#sellinkfield").length>0)
    ...
    function showhide(typeval) {
        var pos = $("#selposition"), icon = $("#lbicon"), link = $("#sellinkfield"), disp = $("#seldisplay"), name = $("#inpbname"), sty = $("#inpbstyle");
       // var command = $("#selaction"), mapping = $("#spmap"), script = $("#txscript");
        
        $([pos, icon, link, disp, name,sty]).each(function (i, k) {
            k.closest("tr").hide();
        })
        var slist = [];
        switch (typeval) {
            case "button":
                slist = [pos, disp, name,sty,link];
                pos.val(position);
                break;
            case "icon":
                slist = [pos, icon, disp,link];
                pos.val(position);
                break;
            case "link":
                slist = [pos,link];
    ...
    function actionlist(dt, action) {
        ...
        var first = true;
        jsonReadAjax("imcdata", "", "code", $("#lbactiondatacode").text(), showhidedatamap, [action]);
    ...
    function showhidedatamap(data1, action1) {
        ...
        var tr1 = $("#lbactiondatacode"), tr2 = $("#spmap"), tr3 = $("#spconnectstring");
        var tr4 = $("#spactiontype"), tr5 = $("#txScript"), tr6 = $("#tdtable");
...
function loadbtnstyle(input, appendto) {
...
function reloadedit(rowdt) {
    ...
    function datasrc(data, rowdt) {
        ...
        var keydata = [[makeCtr(["span", "Parent", , , ]), makeCtr(["span", "Child", , , ]), makeCtr(["span", "DefValue", , , ]),'']];
        if (keyarray.length > 0) {
            $(keyarray).each(function (i, k) {
                keydata.push([k, makeCtr(["select", field1.join(";") + ";custom", "", "", ]), makeCtr(["input", , "", "", ]), makeCtr(["i", "fa fa-times-circle imdim", , "", ""])]);
            });
        }
        foot = ['<span id="inpaddrow" style="border:solid 1px lightslategray" class="btnRoundsmall imdim"><i class="fa fa-plus-circle"/>add</span>' +
                '|{"colspan":"4","style":"text-align:right;padding:5px;"}'];
      
        var tbkeymap = makeTable("tbkeymap", keydata, "general");
        tbkeymap = appendFooter(tbkeymap, foot);
        ...
        var tb = $("<table />"), tr = $("<tr />"), td = $("<td style='width:110px'/>");
        dv.append(tb);
        ...
        td = $("<td />"); tr.append(td); td.append($("<textarea class='form-control' style='" + sty + "' id='txFunc'/>"));
        ...
        td = $("<td />"); tr.append(td); td.append($("<div class='form-inline'><input class='form-control col-10 marginright' value='600'/><span class='col-1'>px</span></div>"));//.append($(makeCtr(["select", "px;%", "", "margin-left:5px", ""])));
        dv.dialog(opt());
        ...
        $("#inpaddrow").on("click", function () {
            var append = ["<input class='form-control'/>", makeCtr(["select", field1.join(";") + ";custom", "", "", ])
                , "<input  class='form-control'/>", makeCtr(["i", "fa fa-times-circle imdim", , "", ""])];
...
function cssList(dt,ctrtype) {
   ...
   // td.append($("<div id='dvcsstojson' style='display:none'><div style='float:left'><label>Class List</label></div></div><div style='clear:both'/>"));
        ...
        td.append($("<div style='text-align:right;padding:3px;'>"+makeCtr(["select", "select,,selected,disabled;"+domlist1.join(';'), "sellisttype", "height:23px;margin-right:5px", ""])
    ...
    $(["Title","Desc","DomType","Style List"]).each(function(i,k){
        ...
        switch(i){
            ...
            case 1:
                ...
                break;
            case 2:
                td.last().append($(makeCtr(["select", "select,,selected,disabled;"+domlist, "seldomtype", "height:23px;margin-right:5px", "onchange:applydomtype($(this).val())"])+makeCtr(["input",,,"display:none",])));
                break;
        }
    });
    $("#inpcssCode").val(code);
    ...
    switch (domtype) {
        case "table":
            ...
            var data = [
             ...
             , ["css", makeCtr(["select", "select,,selected,disabled;normal;italic;inherit;initial;oblique", "selstyle", "inp", ""])]
...
function styleBuild(style, showtext) {
    ...
    var data = [
        [makeCtr(["span", "Option", , , ]), makeCtr(["span", "Value", , , ])]//headers
       , ["font-family", makeCtr(["select", "select,,selected,disabled;dotum;malgun gothic,malgun_gothic;gulim;gothic;Helvetica;Verdana;sans-serif;Times New Roman,Times_New_Roman;Palatino;Bookman;serif", "selfontfamily", "inp", chg])]
       ...
       , ["font-style", makeCtr(["select", "select,,selected,disabled;normal;italic;inherit;initial;oblique", "selstyle", "inp", chg])]
       , ["color", makeCtr(["input:color", "", "inpcolor", "width:150px", chg])]
        , ["display", makeCtr(["select", "select,,selected,disabled;inline;block;none", "seldisplay", "inp", chg])]
        , ["visibility", makeCtr(["select", "select,,selected,disabled;visible;hidden;collapse", "selvisibility", "inp", chg])]
       ...
       , ["vertical-align", makeCtr(["select", "select,,selected,disabled;top;middle;bottom", "selverticalalign", "inp", chg])]
       , ["horizontal-align", makeCtr(["select", "select,,selected,disabled;left;center;right", "selhorizontalalign", "inp", chg])]
       , ["text-align", makeCtr(["select", "select,,selected,disabled;left;center;right", "seltextalign", "inp", chg])]
       , [makeCtr_img("inpborder", 7) + "border", makeCtr(["input", "", "inpborder", "width:100px", kup1])]
       , ["border-style", makeCtr(["select", "select,,selected,disabled;none;hidden;dotted;dashed;solid;double;groove;ridge;inset;outset;initial;inherit", "selborderstyle", "inp", chg])]
       ...
       , ["border-top-style", makeCtr(["select", "select,,selected,disabled;none;hidden;dotted;dashed;solid;double;groove;ridge;inset;outset;initial;inherit", "selbordertopstyle", "inp", chg])]
       ...
       , ["border-right-style", makeCtr(["select", "select,,selected,disabled;none;hidden;dotted;dashed;solid;double;groove;ridge;inset;outset;initial;inherit", "selborderrightstyle", "inp", chg])]
       ...
       , ["border-bottom-style", makeCtr(["select", "select,,selected,disabled;none;hidden;dotted;dashed;solid;double;groove;ridge;inset;outset;initial;inherit", "selborderbottomstyle", "inp", chg])]
       ...
       , ["border-left-style", makeCtr(["select", "select,,selected,disabled;none;hidden;dotted;dashed;solid;double;groove;ridge;inset;outset;initial;inherit", "selborderleftstyle", "inp", chg])]
...
function helpEditTable(id, gdt) {
    ...
    var urls = [];
    urls.push("select,,selected,disabled");
...
function helpshow(code,content) {
    ...
    function editmode(dv, content, hrcode) {
       ...
       //if(code=="") codeinsert="<input id='inpcode' style='width:80%'></input>";
        dv.append($("<div class='form-group row'><label class='col-2 col-form-label'>Code</label>"+codeinsert+"<div/></div>"));
        dv.append($("<div class='form-group row'><label class='col-2 col-form-label'>Title</label><input id='inptitle' class='form-control col-10'></input><div/></div>"));
        dv.append($("<div class='form-group row'><label  class='col-2 col-form-label'>Content</label><textarea class='form-control' id='txHelp'/></div>"));
